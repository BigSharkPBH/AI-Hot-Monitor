...
| AI 服务 | OpenRouter → deepseek/deepseek-v3.2 | 低成本，中文能力强 |
...
| 数据源 | 内容类型 | 接入方式 | 备注 |
|--------|----------|----------|------|
| Twitter/X | 实时推文、热议话题 | twitterapi.io REST API | 需要 API Key，内置质量过滤 |
| Hacker News | 技术热帖、评论 | 官方 Firebase API（免费） | 无需 Key，同时抓取 top+new |
| RSS 聚合 | TechCrunch AI、ArXiv CS.AI、MIT Tech Review | XML RSS 解析 | 无需 Key |
| GitHub Trending | 热门 AI/编程项目 | 页面爬取 | 无需 Key |
| SearXNG | 多搜索引擎聚合新闻 | 公共实例 REST API | 无需 Key，多实例 fallback |
| Reddit | 技术社区讨论 | 公共 JSON API | 无需 Key，搜索+热门 subreddit |

#### Twitter 质量过滤策略

为避免低质量推文污染数据，Twitter 数据源内置以下硬过滤规则：
...
### 3.3 AI 功能设计（deepseek/deepseek-v3.2 via OpenRouter）
...
│   │   │   └── sources/
│   │   │       ├── twitter.js     # Twitter/X 抓取（含质量过滤）
│   │   │       ├── hackernews.js  # Hacker News 抓取（top+new）
│   │   │       ├── rss.js         # RSS 聚合解析
│   │   │       ├── github.js      # GitHub Trending 抓取
│   │   │       ├── searxng.js     # SearXNG 元搜索引擎
│   │   │       └── reddit.js      # Reddit 社区热帖
│   │   ├── services/
│   │   │   ├── collector.js       # 聚合采集器
...
1. **技术栈**：React + Vite（前端）、Node.js + Express（后端）、SQLite（数据库）——✅ 已确认
2. **AI 模型**：deepseek/deepseek-v3.2 via OpenRouter——✅ 已确认
3. **数据源**：Twitter/X + HackerNews + RSS + GitHub Trending + SearXNG + Reddit——✅ 已确认（Twitter 内置质量过滤：点赞≥50/转发≥20/浏览≥2000，仅原创推文）
4. **通知方式**：浏览器 Push + 邮件——✅ 已确认
5. **UI 风格**：「Signal」暗黑赛博风格——✅ 已确认
6. **采集频率**：默认每 30 分钟采集一次——✅ 已确认
7. **OpenRouter API Key**：✅ 已配置
